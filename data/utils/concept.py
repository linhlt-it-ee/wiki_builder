...
def create_document_concept_graph(
    doc_ids: List[str], doc_mention: Dict, mention_concept: Dict, par_num: List[int]
):
    doc_ids = set(doc_ids)
    all_mentions = set(
        [e for did, mention in doc_mention.items() if did in doc_ids for e in mention]
    )
    ...
    for level, parlevel_num in enumerate(par_num, start=1):
        ...
        children = [
            k for k, v in sorted(cnt.items(), key=lambda x: (x[1], x[0]), reverse=True) if v >= 2
        ][:parlevel_num]
        C.update(children)
        print(
            f"Extracting {parlevel_num} parents level {level} with most children, got {len(children)}"
        )
