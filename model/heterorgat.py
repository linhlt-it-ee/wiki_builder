...
class HeteroRGATLayer(nn.Module):
    def __init__(self, in_feat: int, out_feat: int, rel_names: List[str]):
        super().__init__()
        self.linears = nn.ModuleDict({rel: nn.GATConv(in_feat, out_feat) for rel in rel_names})
...
class HeteroRGAT(nn.Module):
    def __init__(self, in_feat: int, hidden_feat: int, n_classes: int, n_layers: int, rel_names: List[str]):
        ...
        self.layers.append(HeteroRGATLayer(in_feat, hidden_feat, rel_names))
        for _ in range(n_layers - 1):
            self.layers.append(HeteroRGATLayer(hidden_feat, hidden_feat, rel_names))
