...
def prepare_graph(pretrained_model_name: str, data_dir: str, **kwargs) -> Union[dgl.DGLGraph, Dict[str, int]]:
    ...
    with open(os.path.join(data_dir, "data.ndjson"), "r") as f:
        for line in tqdm(f, desc="Featurizing documents"):
            ...
            if id in document_ids:
                ...
                document_feats[doc_id] = model_util.get_text_embedding(model, tokenizer, doc["title"], device=device)
...
def prepare_model(graph: dgl.DGLGraph, out_dim: int, hidden_dim: int = 512, **kwargs) -> nn.Module:
    model_dims = {"in_dims": {}, "hid_dims": {}, "out_dims": {}}
    for ntype in graph.ntypes:
        model_dims["in_dims"][ntype] = graph.ndata["feature"][ntype].shape[1]
        model_dims["hid_dims"][ntype] = hidden_dim
