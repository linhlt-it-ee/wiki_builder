...
def get_kmean_matrix(embeddings: List, num_cluster_list: List[int] = [2], is_gmm: bool = False):
    range_n_clusters = num_cluster_list  # [2, 3, 4, 5, 6]

    for i, num_clusters in enumerate(range_n_clusters):
        if not is_gmm:
            clustering_model = KMeans(n_clusters=num_clusters)
            cluster_distance = clustering_model.fit_transform(embeddings).min(axis=1)
