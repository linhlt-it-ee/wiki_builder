...
from .utils import excel_tree_level_export
from .utils import graph_builder_from_wiki as wiki_graph_utils
from .utils import text_utils, wiki_utils
...
def update_entity_description_shortname(input_dict, all_entities_mention_dict):
    ...
    for entity_id in input_dict:
        ...
        dict_info[entity_id] = {
            ...
            "short_names": entitiy_shortnames,
            "name_mention": all_entities_mention_dict[entity_id]["name_mention"]
            if entity_id in all_entities_mention_dict
            else [],
        }
        new_file.write(
            entity_id + ";" + entity_label + ";" + entity_desc + ";" + str(entitiy_shortnames)
        )
        print(count, "/", total)
        count += 1
    new_file.close()
...
def update_other_children_nodes(parent_of_leaf_nodes, link_data_dict, all_entities_info_dict):
    for i, leaf_parent_id in enumerate(parent_of_leaf_nodes):
        ...
        child_ids, child_labels = wiki_util.get_entities_SPARQ_by_property(
            leaf_parent_id, "P279", is_forward=False
        )
        instance_of_ids, instance_of_labels = wiki_util.get_entities_SPARQ_by_property(
            leaf_parent_id, "P31", is_forward=False
        )
        ...
        for instance_of_id in instance_of_ids:
            ...
            parent_of_instances = [
                x for x in parent_of_instances if x not in link_data_dict[instance_of_id]
            ]
...
if __name__ == "__main__":
    ...
    if not choice:  # choice=0 folder_name, start, end, iteration
        ...
        search_wiki_with_threads(
            sys.argv[1:][1], int(sys.argv[1:][2]), int(sys.argv[1:][3]), int(sys.argv[1:][4])
        )
    elif choice == 1:
